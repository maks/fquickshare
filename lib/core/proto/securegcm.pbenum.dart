...
class AppleDeviceDiagonalMils extends $pb.ProtobufEnum {
  ...
  static const AppleDeviceDiagonalMils APPLE_PHONE =
      AppleDeviceDiagonalMils._(4000, _omitEnumNames ? '' : 'APPLE_PHONE');

  /// This is the mils diagonal on an iPad mini.
  static const AppleDeviceDiagonalMils APPLE_PAD =
      AppleDeviceDiagonalMils._(7900, _omitEnumNames ? '' : 'APPLE_PAD');

  static const $core.List<AppleDeviceDiagonalMils> values =
      <AppleDeviceDiagonalMils>[
    APPLE_PHONE,
    APPLE_PAD,
  ];

  static final $core.Map<$core.int, AppleDeviceDiagonalMils> _byValue =
      $pb.ProtobufEnum.initByValue(values);
  static AppleDeviceDiagonalMils? valueOf($core.int value) => _byValue[value];

  const AppleDeviceDiagonalMils._(super.value, super.name);
}
...
class SoftwareFeature extends $pb.ProtobufEnum {
  static const SoftwareFeature UNKNOWN_FEATURE =
      SoftwareFeature._(0, _omitEnumNames ? '' : 'UNKNOWN_FEATURE');
  static const SoftwareFeature BETTER_TOGETHER_HOST =
      SoftwareFeature._(1, _omitEnumNames ? '' : 'BETTER_TOGETHER_HOST');
  static const SoftwareFeature BETTER_TOGETHER_CLIENT =
      SoftwareFeature._(2, _omitEnumNames ? '' : 'BETTER_TOGETHER_CLIENT');
  static const SoftwareFeature EASY_UNLOCK_HOST =
      SoftwareFeature._(3, _omitEnumNames ? '' : 'EASY_UNLOCK_HOST');
  static const SoftwareFeature EASY_UNLOCK_CLIENT =
      SoftwareFeature._(4, _omitEnumNames ? '' : 'EASY_UNLOCK_CLIENT');
  static const SoftwareFeature MAGIC_TETHER_HOST =
      SoftwareFeature._(5, _omitEnumNames ? '' : 'MAGIC_TETHER_HOST');
  static const SoftwareFeature MAGIC_TETHER_CLIENT =
      SoftwareFeature._(6, _omitEnumNames ? '' : 'MAGIC_TETHER_CLIENT');
  static const SoftwareFeature SMS_CONNECT_HOST =
      SoftwareFeature._(7, _omitEnumNames ? '' : 'SMS_CONNECT_HOST');
  static const SoftwareFeature SMS_CONNECT_CLIENT =
      SoftwareFeature._(8, _omitEnumNames ? '' : 'SMS_CONNECT_CLIENT');
  ...
  static final $core.List<SoftwareFeature?> _byValue =
      $pb.ProtobufEnum.$_initByValueList(values, 8);
  static SoftwareFeature? valueOf($core.int value) =>
      value < 0 || value >= _byValue.length ? null : _byValue[value];

  const SoftwareFeature._(super.value, super.name);
}
...
class InvocationReason extends $pb.ProtobufEnum {
  static const InvocationReason REASON_UNKNOWN =
      InvocationReason._(0, _omitEnumNames ? '' : 'REASON_UNKNOWN');

  /// First run of the software package invoking this call
  static const InvocationReason REASON_INITIALIZATION =
      InvocationReason._(1, _omitEnumNames ? '' : 'REASON_INITIALIZATION');

  /// Ordinary periodic actions (e.g. monthly master key rotation)
  static const InvocationReason REASON_PERIODIC =
      InvocationReason._(2, _omitEnumNames ? '' : 'REASON_PERIODIC');

  /// Slow-cycle periodic action (e.g. yearly keypair rotation???)
  static const InvocationReason REASON_SLOW_PERIODIC =
      InvocationReason._(3, _omitEnumNames ? '' : 'REASON_SLOW_PERIODIC');

  /// Fast-cycle periodic action (e.g. daily sync for Smart Lock users)
  static const InvocationReason REASON_FAST_PERIODIC =
      InvocationReason._(4, _omitEnumNames ? '' : 'REASON_FAST_PERIODIC');

  /// Expired state (e.g. expired credentials, or cached entries) was detected
  static const InvocationReason REASON_EXPIRATION =
      InvocationReason._(5, _omitEnumNames ? '' : 'REASON_EXPIRATION');

  /// An unexpected protocol failure occurred (so attempting to repair state)
  static const InvocationReason REASON_FAILURE_RECOVERY =
      InvocationReason._(6, _omitEnumNames ? '' : 'REASON_FAILURE_RECOVERY');

  /// A new account has been added to the device
  static const InvocationReason REASON_NEW_ACCOUNT =
      InvocationReason._(7, _omitEnumNames ? '' : 'REASON_NEW_ACCOUNT');

  /// An existing account on the device has been changed
  static const InvocationReason REASON_CHANGED_ACCOUNT =
      InvocationReason._(8, _omitEnumNames ? '' : 'REASON_CHANGED_ACCOUNT');

  /// The user toggled the state of a feature (e.g. Smart Lock enabled via BT)
  static const InvocationReason REASON_FEATURE_TOGGLED =
      InvocationReason._(9, _omitEnumNames ? '' : 'REASON_FEATURE_TOGGLED');

  /// A "push" from the server caused this action (e.g. a sync tickle)
  static const InvocationReason REASON_SERVER_INITIATED =
      InvocationReason._(10, _omitEnumNames ? '' : 'REASON_SERVER_INITIATED');

  /// A local address change triggered this (e.g. GCM registration id changed)
  static const InvocationReason REASON_ADDRESS_CHANGE =
      InvocationReason._(11, _omitEnumNames ? '' : 'REASON_ADDRESS_CHANGE');

  /// A software update has triggered this
  static const InvocationReason REASON_SOFTWARE_UPDATE =
      InvocationReason._(12, _omitEnumNames ? '' : 'REASON_SOFTWARE_UPDATE');

  /// A manual action by the user triggered this (e.g. commands sent via adb)
  static const InvocationReason REASON_MANUAL =
      InvocationReason._(13, _omitEnumNames ? '' : 'REASON_MANUAL');

  /// A custom key has been invalidated on the device (e.g. screen lock is
  /// disabled).
  static const InvocationReason REASON_CUSTOM_KEY_INVALIDATION =
      InvocationReason._(
          14, _omitEnumNames ? '' : 'REASON_CUSTOM_KEY_INVALIDATION');

  /// Periodic action triggered by auth_proximity
  static const InvocationReason REASON_PROXIMITY_PERIODIC =
      InvocationReason._(15, _omitEnumNames ? '' : 'REASON_PROXIMITY_PERIODIC');
  ...
  static final $core.List<InvocationReason?> _byValue =
      $pb.ProtobufEnum.$_initByValueList(values, 15);
  static InvocationReason? valueOf($core.int value) =>
      value < 0 || value >= _byValue.length ? null : _byValue[value];

  const InvocationReason._(super.value, super.name);
}
...
class Type extends $pb.ProtobufEnum {
  ...
  static const Type TX_SYNC_REQUEST =
      Type._(4, _omitEnumNames ? '' : 'TX_SYNC_REQUEST');
  static const Type TX_SYNC_RESPONSE =
      Type._(5, _omitEnumNames ? '' : 'TX_SYNC_RESPONSE');
  static const Type TX_PING = Type._(6, _omitEnumNames ? '' : 'TX_PING');
  static const Type DEVICE_INFO_UPDATE =
      Type._(7, _omitEnumNames ? '' : 'DEVICE_INFO_UPDATE');
  static const Type TX_CANCEL_REQUEST =
      Type._(8, _omitEnumNames ? '' : 'TX_CANCEL_REQUEST');

  /// DEPRECATED (can be re-used after Aug 2015)
  static const Type PROXIMITYAUTH_PAIRING =
      Type._(10, _omitEnumNames ? '' : 'PROXIMITYAUTH_PAIRING');
  ...
  /// key)
  static const Type GCMV1_IDENTITY_ASSERTION =
      Type._(11, _omitEnumNames ? '' : 'GCMV1_IDENTITY_ASSERTION');
  ...
  static const Type DEVICE_TO_DEVICE_RESPONDER_HELLO_PAYLOAD = Type._(
      12, _omitEnumNames ? '' : 'DEVICE_TO_DEVICE_RESPONDER_HELLO_PAYLOAD');
  ...
  static const Type DEVICE_TO_DEVICE_MESSAGE =
      Type._(13, _omitEnumNames ? '' : 'DEVICE_TO_DEVICE_MESSAGE');

  /// Notification to let a device know it should contact a nearby device.
  static const Type DEVICE_PROXIMITY_CALLBACK =
      Type._(14, _omitEnumNames ? '' : 'DEVICE_PROXIMITY_CALLBACK');
  ...
  static const Type UNLOCK_KEY_SIGNED_CHALLENGE =
      Type._(15, _omitEnumNames ? '' : 'UNLOCK_KEY_SIGNED_CHALLENGE');

  /// Specialty (corp only) features
  static const Type LOGIN_NOTIFICATION =
      Type._(101, _omitEnumNames ? '' : 'LOGIN_NOTIFICATION');
